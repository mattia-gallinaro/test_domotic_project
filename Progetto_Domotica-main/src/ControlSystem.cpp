...
void ControlSystem::read_devices_from_file(){
    ...
    std::stringstream ss;
    int i = 0;
    std::ifstream FileReader("../include/dispositivi.txt");
    while(std::getline(FileReader, text)){
        ...
        switch (current_device[1] == "Manuale")
        {
            ...
            case false:{
                devices.push_back(new ProgrammedCycle(current_device[0], std::stoi(current_device[1]), std::stod(current_device[2])));
            break;
            }
        }
        ss.clear();
...
double ControlSystem::get_current_contribution(){
    ...
    for(auto i : devices)if(i->get_status())sum_contribution += i->get_contribuition();
...
void ControlSystem::order_devices(){
...
void ControlSystem::reset_timers(){
    ...
    for(auto i : devices){
        i->reset_timer(house_time);
        if(static_cast<ProgrammedCycle*>(i) != nullptr && i->get_status()) add_action_to_vector( i->get_name(),house_time + static_cast<ProgrammedCycle *>(i)->work_period, false, true);
    }
}
...
std::string ControlSystem::remove_timer(const std::string& name){
    ...
    if(pos != -1){// se lo trovo allora in pos ho la posizione nell'array del dispositivo richiesto
        ...
        if(static_cast<ProgrammedCycle*>(devices[pos]) != nullptr) add_action_to_vector(devices[pos]->get_name(), static_cast<ProgrammedCycle*>(devices[pos])->work_period + house_time, false, true);
        return output;
    }
...
std::string ControlSystem::show_device(const std::string& name){
    ...
    if(name == impianto.get_name()) energy_consumed = impianto.get_energy_consumed();
    else{
        int pos = find_device_by_name(name);
...
std::string ControlSystem::show_all(){
    ...
    output = "["+ std::to_string(house_time) + "] Attualmente il sistema ha prodotto " + std::to_string((impianto.get_energy_consumed() + rete_elettrica * (double)(house_time / 60))) 
...
std::string ControlSystem::set_device_on(const std::string& name){
    ...
    if(pos == -1)throw std::invalid_argument("Il nome inserito non è presente tra i dispositivi disponibili");
        ...
        if(!devices[pos]->get_status() && static_cast<ProgrammedCycle*>(devices[pos]) != nullptr) add_action_to_vector(devices[pos]->get_name(),house_time + static_cast<ProgrammedCycle *>(devices[pos])->work_period, false, true);
        return devices[pos]->setOn(house_time);
    }
}
...
std::string ControlSystem::set_time_of_day(const std::string& time_to_reach){
    ...
    while(house_time < end_time && house_time < 1440){ //1440 corrisponde alla mezzanotte del giorno successivo
        if(action_timestamp.size() > 0){
            int i = 0;
            while(action_timestamp[i].time_action == house_time){
                if(action_timestamp[i].to_activate){
                    output += set_device_on(action_timestamp[i].name);
                }else{
                    output += set_device_off(action_timestamp[i].name);
                }
                if(get_current_contribution() < 0) output += turn_off_devices();
                if(action_timestamp[0].destoyable_action){
                    action_timestamp.erase(action_timestamp.begin() + i, action_timestamp.begin() + 1 + i);
                    i--;
                }
                i++;
            }
...
std::string ControlSystem::set_timer_device(const std::string& name , const std::string& start, const std::string& end){
    ...
    if(name == impianto.get_name()) {
        ...
        add_action_to_vector(impianto.get_name(), start_time, true);
        if(end_time != -1)add_action_to_vector(impianto.get_name(), end_time, false);
        return output;
    }
    int pos = find_device_by_name(name);
    if(pos == -1) throw std::invalid_argument("Il dispositivo richiesto non esite");
    if(end_time == -1 && static_cast<ProgrammedCycle*>(devices[pos]) != nullptr)end_time = house_time + static_cast<ProgrammedCycle *>(devices[pos])->work_period;
    if(end_time == -1 ){
        ...
        if(dynamic_cast<ProgrammedCycle*>(devices[pos]) != nullptr){
            ...
            add_action_to_vector(devices[pos]->get_name(), start_time, true);
            add_action_to_vector(devices[pos]->get_name(), start_time + static_cast<ProgrammedCycle *>(devices[pos])->work_period, false, true);
        }else{
            output = devices[pos]->setTimer(house_time, start_time, end_time);
            add_action_to_vector(devices[pos]->get_name(), start_time, true);
        ...
        case 4:
            ...
            if(command_inserted[command_inserted.size() -1].find(":") != std::string::npos){
                ...
                if(command_inserted[1] != "time"){//chiamo funzione con set device e i due time
                    output += set_timer_device(command_inserted[1], command_inserted[command_inserted.size() - 2], command_inserted[command_inserted.size() - 1]);
                }
...
void ControlSystem::remove_destroyable_timers(){
    for(int i = 0; i < action_timestamp.size();i++){
        if(action_timestamp[i].destoyable_action){
            action_timestamp.erase(action_timestamp.begin() + i, action_timestamp.begin() + i + 1);
            i--;
        }
    }
}
